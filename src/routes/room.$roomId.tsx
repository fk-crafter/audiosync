...
function RoomComponent() {
  ...
  return (
    <div className="mx-auto flex h-dvh max-w-6xl flex-col bg-stone-900 p-2 md:p-6 lg:p-8">
      <header className="mb-2 flex shrink-0 items-center justify-between gap-2 rounded-2xl border border-stone-700 bg-stone-800 p-3 shadow-lg md:mb-6 md:rounded-3xl md:p-5">
        <div className="flex min-w-0 flex-col">
          <h1 className="truncate text-base font-semibold tracking-tight text-stone-100 md:text-xl">
            Salon : <span className="text-stone-300">{roomId}</span>
          </h1>
          <p className="truncate text-xs text-stone-400 md:text-sm">
            Connecté :{' '}
            <span className="font-medium text-stone-200">{username}</span>
          </p>
        </div>
        <button
          onClick={copyInviteLink}
          className={`flex shrink-0 items-center justify-center gap-2 rounded-full p-2.5 text-sm font-medium transition-all md:px-5 ${
            isCopied
